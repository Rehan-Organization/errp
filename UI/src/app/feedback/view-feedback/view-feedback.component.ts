...
export class ViewFeedbackComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    fetchFeedbacks()
    {
      ...
      {
        this.flag=0;
        this.feedbackService.fetchAllFeedbacks(1).subscribe(feedback=>this.feedbacks=feedback);
      }
      else{
        this.flag=1;
        this.feedbackService.fetchAllFeedbacks(2).subscribe(feedback=>this.feedbacks=feedback);
      }
    }
    cleanFeedback()
    {
      for(var i=0;i<this.feedbacks.length;i++)
      {
        ...
        {
          this.feedbackService.fetchEmployee(this.feedbacks[i].senderId).subscribe(Employee=>this.map.set(Employee.employeeId,Employee.employeeName));
        }
      }
    }
    addFeedback()
        ...
        this.feedbackService.removeFeedback(feedback.feedbackId).subscribe(feedback=>
          {
            ...
            else
            {
              ...
              // this.fetchFeedbacks();      
              this.feedbacks = this.feedbacks.filter(newFeedback => newFeedback.feedbackId != feedback.feedbackId);
            }
          });
        }
      }
